$(document).ready(function () {
  ...
  socket.on("render_uploaded_csv_data", function (msg) {
    ...
    var original_chart = document
      .getElementById("originalChart")
      .getContext("2d");
  ...
  socket.on("render_forecast_chart", function (msg) {
    ...
    var time_to_render_forecast = (
      (forecast_render_time - generate_forecast_time) /
      1000
    ).toFixed(2);
    ...
    $("#update-chart").click(function () {
      ...
      var forecast_settings_list = [
        u_model_type,
        u_forecast_length,
        u_capacity,
        u_min_saturation,
        u_seasonality_mode,
        u_seasonality_prior_scale,
  ...
  socket.on("error", function (msg) {
    ...
    $("#error-message").html(
      "<b>Error: </b> Whoops! You may want to check the following rows in your csv for null values: <br/>" +
        "Row #: " +
        error_data
    );
  });
